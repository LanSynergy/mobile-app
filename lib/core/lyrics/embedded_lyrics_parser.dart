...
class EmbeddedLyricsParser {
  ...
  static Future<String?> _extractLyricsFromMp3(File file) async {
    ...
    try {
      ...
      final maxReadSize = tagSize < 10 * 1024 * 1024 ? tagSize : 10 * 1024 * 1024;
      ...
      while (i < buffer.length - 10) {
        ...
        if (buffer[i] == 0x55 && buffer[i + 1] == 0x53 &&
            buffer[i + 2] == 0x4C && buffer[i + 3] == 0x54) {
  ...
  static String? _parseUsltFrame(Uint8List buffer, int startIndex, int size) {
    ...
    if (encoding == 0 || encoding == 3) {
      ...
      while (textStart + 1 < bodyStart + size && !(buffer[textStart] == 0 && buffer[textStart + 1] == 0)) {
        textStart += 2;
      }
  ...
  static Future<String?> _extractLyricsFromFlac(File file) async {
    ...
    try {
      ...
      while (!isLast) {
        ...
        final length = (blockHeader[1] << 16) | (blockHeader[2] << 8) | blockHeader[3];
  ...
  static String? _parseVorbisComment(Uint8List buffer) {
    ...
    for (int i = 0; i < commentCount; i++) {
      ...
      final commentStr = utf8.decode(buffer.sublist(offset, offset + commentLen), allowMalformed: true);
      offset += commentLen;
  ...
  static Future<String?> _scanM4aAtoms(RandomAccessFile raf, int maxBytes) async {
    ...
    while (maxBytes < 0 || (await raf.position() - startPosition) < maxBytes) {
      ...
      if (type == 'moov' || type == 'udta' || type == 'meta' || type == 'ilst') {
        ...
        final dSize = (dataHeader[0] << 24) | (dataHeader[1] << 16) | (dataHeader[2] << 8) | dataHeader[3];
