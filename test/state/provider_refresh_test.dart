...
void main() {
  group('Riverpod Invalidation Race Condition (BUG-3)', () {
    test(
      'Sequence A (Buggy): read future then invalidate resolves immediately with stale data',
      () async {
        int fetchCount = 0;
    ...
    test(
      'Sequence B (Correct): invalidate then read future waits for the new fetch to complete',
      () async {
        int fetchCount = 0;
        final completerProvider = FutureProvider<int>((ref) async {
          fetchCount++;
