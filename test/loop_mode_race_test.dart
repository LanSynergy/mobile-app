...
void main() {
  group('Loop mode race conditions', () {
    ...
    group('setAfLoopMode lock serialization', () {
      test(
        'setAfLoopMode should serialize setLoop under queueLock to '
        'prevent interleaving with completed handler',
        () async {
          final lock = AfAsyncLock();
          final executionOrder = <String>[];

          // Simulate completed handler entering lock
          final completedFut = lock.run(() async {
            await Future<void>.delayed(const Duration(milliseconds: 10));
            executionOrder.add('completedHandler');
          });

          // Simulate setAfLoopMode WITHOUT lock wrapping setLoop
          // (current buggy behavior — setLoop is outside lock)
          // In the buggy version, this runs immediately without waiting
          executionOrder.add('setLoop_outside_lock');

          // Then setAfLoopMode enters lock for jumpAndPlay
          final setLoopFut = lock.run(() async {
            await Future<void>.delayed(Duration.zero);
            executionOrder.add('setLoopAction_inside_lock');
          });

          await Future.wait([completedFut, setLoopFut]);

          // Buggy order: setLoop_outside_lock can interleave between
          // the completed handler's event and its lock action
          expect(executionOrder[0], 'setLoop_outside_lock');
          expect(executionOrder[1], 'completedHandler');
          expect(executionOrder[2], 'setLoopAction_inside_lock');

          // If setLoop were under the lock, the order would be:
          // completedHandler, setLoopAction (with setLoop inside)
        },
      );

      test(
        'setAfLoopMode under lock preserves correct ordering with '
        'completed handler',
        () async {
          final lock = AfAsyncLock();
          final executionOrder = <String>[];

          // Fix: setAfLoopMode wraps everything under lock, so
          // setLoop is serialized with completed handler.
          ...
          // Simulate setAfLoopMode entering lock (includes setLoop inside)
          final setLoopFut = Future<void>(() async {
          ...
          // setLoop cannot interleave because it's inside the lock.
          await Future.wait([completedFut, setLoopFut]);

          // With the fix, setLoop is serialized:
          expect(executionOrder[0], 'completedHandler');
          expect(executionOrder[1], 'setLoopAction_inside_lock');
        },
      );
    });
  });
